...
function initializeElements() {
    const ids = [
        ...
        'validationBtn', 'helpBtn', 'themeToggle', 'themeTrack', 'tabPrompt', 'tabParams',
        ...
        'handoffsContent', 'helpModalOverlay', 'helpModalClose', 'assistantModalOverlay',
...
async function handleAgenteChange() {
    ...
    try {
        ...
        const response = await fetchAPI(`/api/agents/${agenteId}`);
        ...
        AppState.editor.value(agentData.system_prompt_es || '');
...
async function loadAgentResources(agenteId) {
    try {
        const [toolsResponse, handoffsResponse] = await Promise.all([
